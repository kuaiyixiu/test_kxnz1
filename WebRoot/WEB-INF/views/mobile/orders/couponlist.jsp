...
var pullupRefresh = function(ul) {
	...
	$.ajax({
		type : 'post',
		url : "wechat/orders/getCouponList.do",
		data : param,
		dataType : 'json',
		async:false,
...
<script id="orderTpl" type="text/html">
    {{each data as coupon i}}
		<li class="mui-table-view-cell">
			<dl>
				<dd>名称：{{coupon.couponName}}</dd>
				<dd>有效期：{{coupon.createTime}} ~ {{coupon.endTime}}</dd>
				<dd>原始金额：{{coupon.totalAmount.toFixed(2)}}元</dd>
				<dd>可用金额：{{coupon.availAmount.toFixed(2)}}元</dd>
				<dd>使用说明：{{coupon.remark}}</dd>
			</dl>
		</li>
    {{/each}}
</script>
