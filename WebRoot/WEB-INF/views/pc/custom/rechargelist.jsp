	...
	var tableIns = table.render({
		...
		cols : [[{
			...
			templet: function(row) {
				return "<a href='javascript:void(0);' onClick='showCustomDetail("+row.custId+")' class='showCustomDetail'>"+row.cardNo+"</a>";
			}
		},{
			field : 'rechargeTime',
			width : '15%',
			...
			title : '充值时间',
			templet: function(row) {
				return formatDateMinutes(row.rechargeTime);
				}
		},{
			field : 'rechargeName',
			width : '15%',
			...
			title : '充值金额',
			templet : function(row) {
				return parseFloat(row.rechargeAmount).toFixed(2);
			}
		},{
			field : 'sendCouponType',
			width : '15%',
			align : 'center',
			title : '赠送',
			templet : function(row) {
				if(row.sendCouponType==1)
					return "代金券";
				else if(row.sendCouponType==2)
					return "代金券包";
				else
					return "无";
			}
