...
<script type="text/javascript">
	layui.use([ 'jquery', 'form', 'laytpl', 'layer' ], function() {
		...
		form.on('submit(formSubmitBtn)', function(data) {
			...
			var data = {
				"id": $("#serveId").val(),
				"name": $("#serveId").find("option:selected").text(),
				"price": $("#serveId").find("option:selected").attr("data-price"),
				"count": $("#count").val()
			}
			
			var tpl = serveTpl.innerHTML;
			laytpl(tpl).render(data, function(html) {
				$(window.parent.document).find("#serveInfo").find("tbody").append(html);
			});
			
			parent.layer.msg("添加服务成功", {
				icon : 1
			});
			...
			<div class="layui-inline">
				<label class="layui-form-label">服务名</label>
				 <div class="layui-input-block normal-select">
				  	<select id="serveId" lay-search="">
				  		<option value=""></option>
					    <c:forEach items="${serveList}" var="item">
					    	<option value="${ item.id }" data-price="${ item.price }">${ item.serveName }</option> 
					    </c:forEach>
				 	 </select>
			     </div>
			</div>
...
<script id="serveTpl" type="text/html">
	<tr class="mealDts" data-type='2'>
		<td>{{ d.name }}</td>
		<td>{{ d.price }}</td>
