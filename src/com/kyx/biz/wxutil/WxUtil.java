...
public class WxUtil {
	 ...
	 * @return
	 */
	public static JSONObject prepareMenus(List<AccountMenu> menus) {
		JSONObject root = new JSONObject();
		if(!CollectionUtils.isEmpty(menus)){
			List<AccountMenu> parentAM = new ArrayList<AccountMenu>();
			Map subAm = new HashMap();
			for(AccountMenu m : menus){
				if (m.getParentId().intValue() == 0) {//一级菜单
					...
					List<JSONObject> tmpMenus = (List<JSONObject>) subAm.get(m.getParentId());
			...
			for(AccountMenu m : parentAM){
				if(subAm.get(m.getId()) != null){//有子菜单
					arr.add(getParentMenuJSONObj(m,(List<JSONObject>)subAm.get(m.getId())));
				}else{//没有子菜单
